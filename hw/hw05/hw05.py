...
def make_withdraw(balance, password):
    ...
    error_times = 3
    locked = False
    def bank_password(amount, action_password):
        nonlocal error_lst, locked,balance

        if locked:
            return f'Too many incorrect attempts. Attempts: {error_lst}'
        if action_password == password:
            if amount > balance:
                return 'Insufficient funds'
            else:
                balance -= amount
            ...
            error_lst.append(action_password)
            if len(error_lst) == error_times:
                locked = True
